...
(1, 2, 3)</code></pre>
          <p>Рассмотрим случай, когда уместно использовать кортежи. В Python изменяемые объекты
            передаются в функцию по ссылке. Это значит, что не создается копия объекта, а переменной-
...
[6, 9, 5, 9]</code></pre>
          <p>То есть исходный список был также изменен. Параметр seq содержал ссылку не на свой 
            локальный список, а на список-оригинал. Таким образом, в операторе return здесь нет смыла. 
...
[6, 9, 5, 9]</code></pre>
          ...
          <p>Второй способ защитить список-оригинал – использовать кортеж. Этот способ более 
...
(1, 'do', ['param', 15, 20])</code></pre>
          ...
          <p>Странная ситуация. Кортеж неизменяем, но мы все-таки можем изменить его. На самом деле 
...
(1, 'do', ['param', 10, 20])</code></pre>
          <p>Кортеж содержит переменную-ссылку. Поменять ее на другую ссылку нельзя. Но кортеж не 
