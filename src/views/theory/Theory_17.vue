...
[16, 46, 26, 36]</code></pre>
          <p>В заголовке цикла for берутся элементы вовсе не списка, а объекта range. Список, элементы
              которого планируется перезаписывать, тут по4сути не фигурирует. Если заранее знать длину
...
3 36</code></pre>
          <p>Если функция enumerate() так хороша, зачем использовать range() в заголовке for? На самом
            ...
            возвращает так называемый объект4итератор. Когда такие объекты сгенерировали значения,
          ...
          <p>Когда range() и enumerate() используются в заголовке for, то разницы нет, так как range4 и
            enumerate4объекты не присваиваются переменным и после завершения работы цикла
